...
def output_format_check(prompt: str, completion: str, example: dict) -> float:
    ...
    try:
        ...
        if match is None or len(match.groups()) != 2:
            logger.warning('output_format_check: Regex did not match or wrong group count')
            return 0.0
        ...
        if len(guess) != 5:
            logger.info(f'output_format_check: Guess length not 5: {guess}')
            return 0.1
        ...
        if guess not in word_list["Word"].values:
            logger.info(f'output_format_check: Guess not in word list: {guess}')
            return 0.5
...
def uses_previous_feedback(prompt: str, completion: str, example: dict) -> float:
    ...
    try:
        ...
        if match is None or len(match.groups()) != 1:
            logger.warning('uses_previous_feedback: Regex did not match or wrong group count')
            return 0.0

        guess = match.groups()[0].strip()
        if len(guess) != 5:
            logger.info(f'uses_previous_feedback: Guess length not 5: {guess}')
            return 0.0
...
def guess_value(prompt: str, completion: str, example: dict) -> float:
    ...
    try:
        ...
        guess = match.groups()[0].strip()
        if len(guess) != 5:
            logger.info(f'guess_value: Guess length not 5: {guess}')
            return 0.0

        # Load the word list
        word_list = pd.read_csv(str(example["word_list"]))
        if guess not in word_list["Word"].values:
            logger.info(f'guess_value: Guess not in word list: {guess}')
            return 0.0
