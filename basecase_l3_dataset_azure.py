...
AZURE_OPENAI_DEPLOYMENT = os.getenv("AZURE_OPENAI_MODEL").strip()
...
logger.info(f"Loaded Azure OpenAI deployment: {AZURE_OPENAI_DEPLOYMENT}")
...
def generate_stream(prompt: str) -> str:
    ...
    completion = azure_client.generate(prompt, max_tokens=50000)
...
def play_game_on_validation(val_secrets):
    logger.info(f"Validating Azure OpenAI model on {len(val_secrets)} secret words.")
...
if __name__ == "__main__":
    logger.info("Azure OpenAI model:")
